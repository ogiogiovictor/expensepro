...
$('#createAccout').click(function(e){
    ...
    if(actName == "" || actCode == "" || Accountcategory == ""){
        $('#showError').html("Please make sure all fields are filled").addClass("alert alert-danger");
    }else{
            
           $('#showError').html("Processing Account, Please wait...");  
           $('#createCategory').hide();
                $.post(action, {actName: actName, actCode: actCode, Accountcategory: Accountcategory},  function(data){
			if(data.msg){
...
$('#confirmchequepay').click(function(e){
    ...
    var CurrencyAmount = document.getElementById("CurrencyAmount");
    var CurrencyType = document.getElementById("CurrencyType");
    
   
//    if(typeof(CurrencyAmount) != 'undefined' && CurrencyAmount != null){
//        if(CurrencyAmount === ""){
//          $('#insurerror').html("Please add Conversion Rate").addClass('errorRed');
//           return;   
//       }  
//    } 

    if(document.body.contains(document.getElementById('CurrencyAmount'))){
        if(CurrencyAmount.value === ""){
           $('#insurerror').html("Please add Conversion Rate<br/>").addClass('errorRed');
           return;   
        }  
    ...
    if(mainAount === "" || chequeManualDate === ""){
        ...
        $.post(action, $('#chequepreparation').serialize(), function (data){ 
...
//This section deals search by date and account officer and category
 ...
 //This section deals search by date and account officer and category
  ...
  $('.approveHotel').click(function () {
      var dataID = $(this).attr('data-id');
      var action = GLOBALS.appRoot + "travelstart/approvehotel";
   
   if(dataID == ""){
        $('#hotelrequestmessage').html("Please Select a Checkbox").addClass("errorRed");
   }else{
         $('#hotelrequestmessage').html("Approving, Please wait...").addClass("errorGreen");
         $.post(action, {dataID: dataID}, function (data){
             
             if(data.msg){
                 $('#hotelrequestmessage').html(data.msg).addClass('alert alert-success');
     ...
     //BEGINNING OF MODAL FOR ACCOUNT PAYABLE
    $('.rejectHotel').click(function (e) {
            var id = $(this).attr('data-id');
            
            var outputs = '<p id="deprocess"><h3 class="btn btn-block btn-fill btn-primary btnblue">Are you Sure you want to reject?</h3></p><textarea class="" rows="3" name="dComment" id="dComment" cols="55" placeholder="This hotel request will be rejected, please enter the reject reason"></textarea><br/><button onClick="rejectrequestforhotel('+id+')" id="rejectRequsition" class="btn btn-sm btn-fill btn-primary">OK</button>';
            $('#myacctputalert').html(outputs);
        });
        
        
 function rejectrequestforhotel(id){
   //$('.forcommentdisplay').show();
   var action = GLOBALS.appRoot+"travelstart/rejecthotel";
    var rejectrequestID = id;
    var dComment = $('#dComment').val();
...
$('.approveHotelbyICU').click(function () {
      var dataID = $(this).attr('data-id');
      var action = GLOBALS.appRoot + "travelstart/approvehotelbyicu";
