...
//This section deals search by date and account officer
  $('#searchbydate').click(function () {
        ...
        if(startDateall =="" || endDateall ==""){
            ...
            $.ajax({
               ...
               method : "POST",
               data: {startDateall: startDateall, endDateall : endDateall},
               dataType : "text",
               success : function (data){
                   $('#results').html(data);
               }
  ...
  $('#catsearchbydate').click(function () {
        ...
        if(catStartDate =="" || catEndDate =="" || acctCode ==""){
            alert("Please choose a Start and End Date and a Category");
         }else{
             $('#results').html('Loading Result, Please wait.....');
            $.ajax({
               url : GLOBALS.appRoot + "reports/categorymainsearch",
               method : "POST",
               data: {catStartDate: catStartDate, catEndDate : catEndDate, acctCode : acctCode},
               dataType : "text",
               success : function (data){
                   $('#results').html(data);
               }
   ...
   if(dAccountName == "" || dAccountGroup ==""){
        $('#showError').html("Please make sure all fields are filled").addClass("errorRed");
         ...
         $.post(action, {dAccountName: dAccountName, dAccountGroup: dAccountGroup}, function (data){
             ...
             if(data.msg){
                 $('#showError').html(data.msg).addClass('alert alert-success');
                 setTimeout(function(){window.top.location= GLOBALS.appRoot + "home/setupaccountants/"} , 100);
             }
...
$('#addBankandactno').click(function(e){
    ...
    var state = $('#state').val();
     ...
     if(bankName == "" || actNumber == "" || address1 == "" || state == ""){
...
$('#bankStatement').click(function(e){
    var action = GLOBALS.appRoot + "home/getthebanksyouwanttoprint/";
    ...
    if(generateStatement !== ""){
        $('#bankerror').html("generating report, please wait...");
         $.post(action, $('#bankselected').serialize(), function (data) {
             setTimeout(function(){window.top.location= action + generateStatement} , 500);
...
$('#sendforreimbursementasforrembursement').click(function(e){
   ...
   if(sendformID == "" || daccountant ==""){
        $('#showcasherror').html("Please make sure all fields are filled").addClass("errorRed");
         ...
         $('#showcasherror').html("Sending Reimbursement Request to Accountantant, Please wait...").addClass("errorGreen");
...
$('#chequeforcashier').click(function(e){
   ...
   if(sentID == "" || dAmount ==""){
        $('#errorCase').html("Please make sure all fields are filled").addClass("errorRed");
         ...
         $('#errorCase').html("Sending Reimbursement Request to Accountantant, Please wait...").addClass("errorGreen");
...
$('#confirmchequepay').click(function(e){
    ...
    var action = GLOBALS.appRoot + "home/confirmchequerequestnownow";
    ...
    if(chequeDate == "" || mainAount == "" || getBank == "" || chequeNo == ""){
        $('#insurerror').html("Please make sure all fields are filled<br/>").addClass('errorRed');
    }else{
        $('#insurerror').html('Making Payment, Please wait.....').addClass('errorRed');
         $.post(action, {vatcharge: vatcharge, witholdingtax: witholdingtax, chequeDate: chequeDate, chequeID: chequeID, chequeNo: chequeNo, payee: payee, mainAount: mainAount, partAmount: partAmount, getBank: getBank}, function (data) {
              if (data.msg) {
                  $('#insurerror').html(data.msg).addClass('alert alert-success')
...
$('#partpaymentbalance').click(function(e){
  ...
  var formIDforpartpay = $('#formIDforpartpay').val();
 ...
 // alert(fullpayment);
  //alert(aAmount);
  if(paypaybalance == "" || newAmountopay =="" || formIDforpartpay=="" || aAmount=="" || newBank =="" || newChequeNo ==""){
     ...
     $('#errorbalance').html("You cannot pay that amount. Total amount must be " + aAmount + " and you have paid " + paypaybalance + "<br/>").addClass('errorRed');  
      ...
      $.post(action, {balancepay: balancepay, newBank: newBank, newChequeNo: newChequeNo, appurL: appurL, userID: userID, aAmount: aAmount, paypaybalance: paypaybalance, formIDforpartpay: formIDforpartpay, newAmountopay: newAmountopay}, function (data) {
              if (data.msg) {
...
$(document).ready(function () {
        ...
        $('.rejectrequestID').click(function (e) {
            ...
            var outputs = '<p id="deprocess">Add Comment</p><textarea class="form-control" rows="2" name="dComment" id="dComment" cols="30" placeholder="Add Comment"></textarea><button onClick="rejectrequest('+id+')" id="approveRequistion" class="btn btn-xs btn-fill btn-primary">OK</button>';
