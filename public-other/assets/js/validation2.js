...
$('#register').click(function (e) {
    ...
    var action = GLOBALS.appRoot + "register/registerUser";
...
$('#btnloginauthenticate').click(function (e) {
    ...
    if (email == "" || pass == "") {
        ...
        $.ajax({
            type: "POST",
            url: GLOBALS.appRoot+"framework/legit/settings/nr/codebase/settings/validate_login",
            data: {
                loginEmail: email,
                loginPass: pass,
...
function loginSuccess(returnedData, status) {
    ...
    if(returnedData.status === 5 && returnedData.accessLevel === "2"){
        $('#email').val('');
        $('#password').val('');
        location.assign(GLOBALS.appRoot + 'userexpense');
        ...
        $('#email').val('');
        $('#password').val('');
        location.assign(GLOBALS.appRoot+'userexpense/generalreportssearch/3');
        $('#ci_error').html('Authenticated, redirecting....').addClass('errorYellow');
         $('#btnloginauthenticate').hide();
    }else if(returnedData.accessLevel == "7"){
        $('#email').val('');
        $('#password').val('');
        location.assign(GLOBALS.appRoot + 'accounts/settings');
        $('#ci_error').html('Authenticated, redirecting....').addClass('errorYellow'); 
         $('#btnloginauthenticate').hide();
    }else if(returnedData.status === 5 && returnedData.accessLevel == "1" || returnedData.accessLevel == "4" || returnedData.accessLevel == "8"){
        $('#email').val('');
        $('#password').val('');
        location.assign(GLOBALS.appRoot + 'home');
        $('#ci_error').html('Authenticated, redirecting....').addClass('errorYellow'); 
        $('#btnloginauthenticate').hide();
        ...
        location.assign(GLOBALS.appRoot + 'phoneNumber/updaterecord');
    }else{
